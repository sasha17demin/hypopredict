...
def identify_hg_events(glucose_df: pd.DataFrame,
                       cgm_only: bool = True,
                       threshold=3.9,
                       min_duration=15) -> pd.DataFrame:
    ...
    glucose_df['hg_group'] = (glucose_df['is_hg'] != glucose_df['is_hg'].shift()).cumsum()
    hg_durations = glucose_df.groupby('hg_group')['is_hg'].transform('sum')
    glucose_df.loc[(glucose_df['is_hg'] == 1) & (hg_durations < (min_duration // 5)), 'is_hg'] = 0
    glucose_df.drop(columns=['hg_group'], inplace=True)
    # record the onsets
    glucose_df['onset'] = (glucose_df['is_hg'] > glucose_df['is_hg'].shift()).astype(int) # if now 1 but was 0 before
...
def plot_hg_events(person):
    ...
    plt.plot(person['hg_events'].index, person['hg_events']['glucose'], label='Glucose Level')
    plt.plot(person['hg_events'].index, person['hg_events']['is_hg'], label='HG indicator')
    # add vertical lines for onsets and ends
    for idx, row in person['hg_events'].iterrows():
        if row['onset'] == 1:
            plt.axvline(x=idx, color='red', linestyle='--', label='HG Onset' if 'HG Onset' not in plt.gca().get_legend_handles_labels()[1] else "")
        if row['end'] == 1:
            plt.axvline(x=idx, color='green', linestyle='--', label='HG End' if 'HG End' not in plt.gca().get_legend_handles_labels()[1] else "")
    plt.axhline(y=3.9, color='yellow', linestyle=':', label='HG Threshold (3.9 mmol/L)')
    plt.xlabel('Time')
    plt.ylabel('Glucose Level (mmol/L)')
    plt.title(f'Person {person["ID"]} Glucose Level with Hypoglycemic Events < 3.9 mmol/L for at Least 15 Minutes')
    plt.legend()
    plt.show()

...
class Person:
    ...
    def load_ECG_day(
        ...
        if len(ecg_day_paths) > 1 and warning:
            print(
                f"""
    WARNING: Multiple files for day _{self.ID}{day}_.
    There might be gaps in concatenated ECG index.
    Check for gaps when verifying HG events during recorded ECG times.
    Files concatenated:
                """,
                ecg_day_paths
            )
    ...
    def _ECG_id_day_paths(self, day: int) -> list:
        ...
        if self.ecg_dir is None:
            raise ValueError(
                "ecg_dir not set. Provide ecg_dir during Person initialization."
            )

        f_paths = []
        for root, _, files in os.walk(self.ecg_dir):
            for file in files:
